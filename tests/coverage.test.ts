...
import { createCoverageReport, formatCoverageReportMarkdown } from "../src/coverage.js";
...
describe("createCoverageReport", () => {
  it("reports fired and never-fired rules for a scan batch", () => {
    ...
    expect(formatCoverageReportMarkdown(coverage)).toContain("# skill-safe Rule Coverage");
    expect(formatCoverageReportMarkdown(coverage)).toContain("## Never Fired");
  });
});
